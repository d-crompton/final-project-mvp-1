...
class TrustedDevices:
    ...
    def __init__(self):
        ...
        for device in self.devices:
            ...
            device_obj.set_type("trusted")
            # Creating Widgets for Device's frame
            inner_frame = Frame(device_frame, bg="#adebad", highlightbackground="black", highlightthickness=1)
            inner_frame.grid(row=row, sticky="WE")
            inner_frame.grid_columnconfigure(0, weight=9)
            inner_frame.grid_columnconfigure(1, weight=1)
            mac_label = Label(inner_frame, text=device_obj.get_mac(), bg="#adebad").grid(row=0, column=0, sticky="W")
            manufacturer_label = Label(inner_frame, text=device_obj.get_manufacturer(), bg="#adebad")
    ...
    def open_file(self):
        ...
        try:
            self.file = open("Files\\trusted.txt", "r")  # Open in read only
